...
3. **PARSE** — reads all snapshots, enriches with GitHub + Anki Forum data, exports to `3-final/` (JSON/Parquet/Excel); re-running PARSE is safe and uses cached raw data. PARSE reprocesses *every* snapshot's cached raw data with the current algorithm, so improving the parsing/export logic regenerates reports for the entire history. PARSE rebuilds `AddonInfos` from `1-raw/` every run rather than reusing the fully-enriched `AddonInfos` that DOWNLOAD already builds in memory (DOWNLOAD discards them). This is intentional: `1-raw/` is the single source of truth, and `SnapshotDir.create()` wipes `2-stage/`/`3-final/` on every DOWNLOAD *and* PARSE run. Reusing DOWNLOAD's output would export the newest snapshot with the download-time algorithm while re-parsed history uses the current one — silently inconsistent. The only duplicated work is CPU-parsing the newest snapshot (no redundant network); DOWNLOAD parses anyway because it must discover GitHub/forum targets from the AnkiWeb pages.
4. **BUNDLE** — zips snapshots, copies finals to `bundle/`, generates HuggingFace dataset card. BUNDLE intentionally re-bundles *all* historical snapshots (not just new ones) so that reports regenerated by an improved PARSE are republished across the full history.
5. **UPLOAD** — syncs `bundle/` to HuggingFace Hub in two non-destructive steps: (1) `upload_dataset` fails fast if write access is missing, then pushes the bundle with the resumable `upload_large_folder` (suited to the multi-GB, growing dataset); (2) `prune_orphans` then deletes only the remote files under `history/`/`latest/` that no longer exist in the local bundle. Upload runs *before* prune, so the published dataset is never left broken mid-run (the old delete-then-upload approach emptied the dataset first and left it broken if the upload failed). `upload_large_folder` has no `delete_patterns`, hence the separate prune step
