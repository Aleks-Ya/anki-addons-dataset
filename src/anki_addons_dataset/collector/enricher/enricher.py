...
class Enricher(Thread):
    ...
    def run(self) -> None:
        while True:
            ...
            if item is self.__sentinel:
                ...
                break
            try:
                enriched_addon_info: AddonInfo = self.__enrich(item)
                self.__addon_infos.append(enriched_addon_info)
    ...
    def wait_finish(self) -> AddonInfos:
        ...
        if self.is_alive():
            ...
            self.__queue.join()
        return self.__addon_infos

    def enrich(self, addon_info: AddonInfo) -> None:
